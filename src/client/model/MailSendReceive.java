    ...
    public static ObservableList<Mail> downloadEmails(User user, PublicParameters publicParameters) throws GeneralSecurityException {
        ...
        try {
            ...
            for (int i = 0; i < arrayMessages.length; i++) {
                ...
                // Store attachment file name, separated by a comma
                String attachFiles = "";
...
private static File decryptFile(MimeBodyPart part, PublicParameters publicParam, User user) throws MessagingException, IOException, ClassNotFoundException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, BadPaddingException, InvalidKeyException {
        ...
        File f = new File("decrypt/"+filename); // Creating a file to save the result of the decryption
        ...
        try {
            ...
            Pairing pairing = PairingFactory.getPairing("src\\utilities\\curves\\a.properties"); // loads the elliptic curve's parameters

            //Decrypt
            byte[] resulting_bytes = IBEBasicIdent.IBEdecryption(pairing, pairing.getG1().newElementFromBytes(publicParam.getP()), pairing.getG1().newElementFromBytes(publicParam.getP_pub()), user.getsK(), cipher); //d�chiffrment Basic-ID IBE/AES

        
            FileOutputStream fout = new FileOutputStream(f);
            fout.write(resulting_bytes); // write decryption result into the file
            fout.close();
        } catch (IOException e) {
            e.printStackTrace();
